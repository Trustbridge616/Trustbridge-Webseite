<template>
  <AppLayout>
    <SeoHead
      title="Warenbox / Abo bestellen – TrustBox Royale"
      description="Wähle deine TrustBox. Starte einmalig oder sammle im Legacy-Abo Punkte und sichere dir Vorabzugang zu besonderen Angeboten."
      keywords="TrustBox bestellen, Legacy Abo, Überraschungsbox kaufen, Retourenware Box"
    ...
    <div class="flow-wrap">
      ...
      <!-- ═══════ THE PREMIUM ORDER STAGE ═══════ -->
      <div class="premium-order-stage" :class="{ 'stage-abo-mode': isAboMode }">
        <div class="stage-ambient-glow" :class="purchaseType"></div>
        ...
        <div class="system-intro-panel">
          ...
          <div class="intro-img-side">
            <transition name="fade" mode="out-in">
              <img v-if="isAboMode" src="/panther-muenze-legacy-nobg.png" key="legacy" class="panther-coin-img" alt="Legacy Panther Münze">
              <img v-else src="/panther-muenze-ohneabo-nobg.png" key="single" class="panther-coin-img" alt="Panther Münze">
            </transition>
          </div>
        </div>

        <!-- STEP 1: LEVEL (3D Plateau) -->
        <div class="stage-section step-1-boxes">
          <div class="step-head">
            <span class="step-badge" :class="isAboMode ? 'badge-abo' : 'badge-single'">1</span>
            <h2>Wähle deine Box</h2>
          </div>

          <div class="plateau-container">
            <div class="plateau-base">
              <div class="plateau-grid-lines" :class="{'grid-abo': isAboMode}"></div>
              <div class="plateau-core-glow" :class="{'core-abo': isAboMode}"></div>
            </div>
            ...
            <div class="level-grid">
              <div
                ...
                :class="{ 
                  ...
                  'abo-active': isAboMode && activeLevel === lvl.id 
                }"
                @click="selectBox(lvl.id)"
              >
                    ...
                    <div class="pu-main">
                      ...
                      <div class="tooltip-wrapper">
                        ...
                        <div class="tooltip-content">
                          <strong style="color: #8EF5D2; display: block; margin-bottom: 6px;">{{ getLevelExplainer(lvl.id) }}</strong>
                ...
                <div class="cc-badge cc-badge-popular" v-if="lvl.id === 3 && !isAboMode">Beste Wahl</div>
                <div class="cc-badge cc-badge-royal" v-if="lvl.id === 5 && !isAboMode">Royal</div>

                <div class="cc-box-stage">
                  <div class="cc-stage-ring"></div>
                  <div class="cc-box-icon">
                    <svg width="56" height="56" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5">
                      <rect x="3" y="3" width="18" height="18" rx="3"/><path d="M3 16l5-5 4 4 4-6 5 7"/>
                    </svg>
                  </div>
                </div>

                <div class="cc-body">
                  <span class="cc-name">Box {{ lvl.id }}</span>
                  <span class="cc-price">{{ lvl.price }} €</span>
                  
                  <div class="cc-value-box">
                    <span class="cc-val-amount">ca. {{ lvl.value }} €</span>
                    <span class="cc-val-label">kalk. Warenwert</span>
                  </div>

                  <span class="cc-packs">{{ lvl.id }} {{ lvl.id === 1 ? 'Warenpaket' : 'Warenpakete' }}</span>
                </div>
              </div>
            </div>

            <!-- Connection Line -->
            <div class="preview-connection-line" :key="`line-${activeLevelData.id}`"></div>

            <!-- Preview Panel -->
              <div class="dynamic-preview-panel activation-pulse" :key="activeLevelData.id">
                <div class="preview-img-wrapper" @click="lightboxOpen = true">
                  <div class="preview-shard-glow"></div>
                  <img :src="`/box_${activeLevelData.id}_example.png`" class="preview-img" alt="Preview Box" />
                ...
                <div class="preview-info">
                  <span class="preview-badge">BEISPIEL-INHALT</span>
                  <h4>Beispiel-Inhalt für Box {{ activeLevelData.id }}</h4>
                  <p class="preview-value">ca. {{ activeLevelData.value }} € kalkulierter Warenwert</p>
                  <p class="preview-disclaimer">Beispiel-Inhalt – tatsächliche Zusammenstellung variiert nach Bestand & Verfügbarkeit.</p>
                </div>
              </div>

            <div class="uvp-explainer-box">
              <div class="uvp-icon">
              ...
              <div class="uvp-text">
                <strong>Kalkulierter Warenwert:</strong> Ein orientierender Vergleichswert der enthaltenen Ware auf Basis von Marktpreisen, Mix und Verfügbarkeit.
              </div>
            </div>
          </div>
        </div>
        ...
        <div class="flow-connector">
          <div class="connector-line" :class="isAboMode ? 'line-abo' : 'line-single'"></div>
          <div class="connector-dot" :class="isAboMode ? 'dot-abo' : 'dot-single'"></div>
        </div>

        <!-- STEP 2: KAUFART -->
        <div class="stage-section step-2-type">
          <div class="step-head">
            <span class="step-badge" :class="isAboMode ? 'badge-abo' : 'badge-single'">2</span>
          ...
          <div class="path-grid">
            <!-- Single -->
            <div
              class="path-card path-single"
              :class="{ active: purchaseType === 'single' }"
              @click="selectType('single')"
            >
              <div class="path-bg"></div>
              <div class="path-border-glow"></div>
              <div class="path-aura"></div>
              
              <div class="path-icon">
                <svg width="32" height="32" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5"><rect x="3" y="3" width="18" height="18" rx="3"/><polyline points="9 12 11 14 15 10"/></svg>
              </div>
              <div class="path-content">
                <h3>Einmalig sichern</h3>
                <p>Klassisch. Ohne Bindung.</p>
              </div>
              <div class="path-check"><svg width="28" height="28" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="3"><polyline points="20 6 9 17 4 12"/></svg></div>
            </div>

            <!-- Abo -->
            <div
              class="path-card path-abo"
              :class="{ active: purchaseType === 'abo' }"
              @click="selectType('abo')"
            >
              <div class="path-bg"></div>
              <div class="path-border-glow"></div>
              <div class="path-aura"></div>
              <span class="path-premium-label">Premium Upgrade</span>
              
              <div class="path-icon">
                <svg width="32" height="32" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5"><path d="M12 2L15.09 8.26L22 9.27L17 14.14L18.18 21.02L12 17.77L5.82 21.02L7 14.14L2 9.27L8.91 8.26L12 2Z"/></svg>
              </div>
              <div class="path-content">
                <h3>Legacy-Abo starten</h3>
                <p>Punkte & Vorabzugang sichern.</p>
              </div>
              <div class="path-check"><svg width="28" height="28" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="3"><polyline points="20 6 9 17 4 12"/></svg></div>
            </div>
          </div>
          ...
          <transition name="fade-slide">
            <div class="abo-benefits-panel" v-if="isAboMode">
              <div class="abp-glow"></div>
              <div class="abp-content">
                <div class="abp-header">
                  <span class="abp-pulse"></span> Legacy-Vorteile für Box {{ activeLevel }} freigeschaltet
                </div>
                <div class="abp-grid">
                  <div class="abp-item">
                  ...
                  <div class="abp-item">
                    <div class="abp-icon">⏱️</div>
                    <div class="abp-text">
                      <strong>48h Vorabzugang</strong>
                      <span>auf besondere Deals</span>
                    </div>
                  </div>
                  <div class="abp-item">
                    <div class="abp-icon">📦</div>
                    <div class="abp-text">
                      ...
                      <span>Frühere Infos zu Schnäppchen</span>
                    </div>
                  </div>
                </div>
            ...
            <div class="microcopy-info" v-else>
              Einmalig = TrustBox testen. Wähle das Legacy-Abo für exklusive Punkte & Vorteile.
            </div>
          </transition>
        </div>
        ...
        <div class="stage-section step-3-checkout">
          <div class="step-head">
            <span class="step-badge" :class="isAboMode ? 'badge-abo' : 'badge-single'">3</span>
            <h2>Deine Auswahl</h2>
          </div>

          <div class="epic-ck-wrapper" :class="[purchaseType, {'checkout-pulse-active-single': checkoutPulseTrigger === 'single', 'checkout-pulse-active-abo': checkoutPulseTrigger === 'abo'}]">
            ...
            <div class="epic-ck-card">
              <div class="epic-ck-top">
                <div class="epic-ck-selection">
                  <span class="epic-ck-box-label">TrustBox Box {{ activeLevel }}</span>
                  <span class="epic-ck-price">{{ activeLevelData.price }} €</span>
                </div>
                <div class="epic-ck-type-pill" :class="purchaseType">
                  {{ purchaseType === 'single' ? 'Einmalig' : 'Legacy-Abo' }}
                </div>
              </div>

              <div class="epic-ck-info" v-if="purchaseType === 'single'">
                Einmalige Bestellung · keine monatliche Bindung
              </div>
              <div class="epic-ck-info ck-info-abo" v-else>
                Legacy-Abo · Punkte-Multiplikator x{{ activeLevel }} · bis zu 48h Deal-Vorabzugang
              </div>

              <button class="epic-cta-btn" :class="purchaseType" @click="checkout">
                <span class="cta-glare"></span>
                <span class="cta-text">{{ purchaseType === 'single' ? 'Einmalig bestellen' : 'Legacy-Abo bestellen' }}</span>
              </button>
            </div>
          </div>
        </div>
    ...
    <div class="vision-video-section">
      <div class="vision-wrap">
        <div class="vision-text">
          ...
          <div class="live-content-teaser">
            <span class="teaser-badge">Bald hier: Live Content</span>
            <h4>Echte Kundenboxen & Verpackungsvideos</h4>
            <p>Sobald unsere ersten Lieferungen gepackt werden, zeigen wir dir hier echte Videos direkt aus dem laufenden Betrieb und vom Packtisch.</p>
          </div>
        </div>
        ...
        <div class="conversion-modal-content" @click.stop>
          ...
          <div class="modal-grid">
            <div class="modal-img-col">
              <img :src="`/box_${activeLevelData.id}_example.png`" class="modal-large-img" :alt="`Beispiel Box Level ${activeLevelData.id}`" />
            </div>
            <div class="modal-text-col">
              <span class="modal-badge">BEISPIEL-INHALT</span>
              <h3 class="modal-title">Beispielbox Level {{ activeLevelData.id }}</h3>
              <p class="modal-subline">So könnte deine TrustBox ungefähr aussehen.</p>
              
              <div class="modal-value-box">
                <span class="val-amount">ca. {{ activeLevelData.value }} €</span>
              ...
              <div class="modal-actions">
                <button @click="selectModeFromModal('single')" class="modal-btn btn-single">
                  <span class="btn-text">Einmalig bestellen</span>
                  <span class="btn-subtext">Ohne Bindung starten</span>
                </button>
                <button @click="selectModeFromModal('abo')" class="modal-btn btn-abo">
    ...
    <div class="faq-section">
      <div class="faq-wrap">
        ...
        <div class="faq-grid">
          <div class="faq-col">
            <div class="faq-item" v-for="(faq, i) in miniFaqs.slice(0, 4)" :key="i" :class="{ open: openFaq === i }">
              ...
              <div class="faq-a-wrap"><div class="faq-a"><p v-html="faq.a"></p></div></div>
            </div>
          </div>
          <div class="faq-col">
            <div class="faq-item" v-for="(faq, i) in miniFaqs.slice(4)" :key="i+4" :class="{ open: openFaq === (i+4) }">
              <button class="faq-q" @click="toggleFaq(i+4)">
                <span>{{ faq.q }}</span>
                <svg class="faq-chev" width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><polyline points="6 9 12 15 18 9"/></svg>
              </button>
              <div class="faq-a-wrap"><div class="faq-a"><p v-html="faq.a"></p></div></div>
...
import SeoHead from '../Components/SeoHead.vue';
  ...
  const checkoutPulseTrigger = ref('');

  const selectModeFromModal = (mode) => {
    purchaseType.value = mode;
    lightboxOpen.value = false;
    setTimeout(() => {
      const checkoutEl = document.querySelector('.premium-order-stage');
      if (checkoutEl) {
        checkoutEl.scrollIntoView({ behavior: 'smooth', block: 'center' });
        
        setTimeout(() => {
          checkoutPulseTrigger.value = mode;
          setTimeout(() => {
            checkoutPulseTrigger.value = '';
          }, 1200);
        }, 600); // Wait for scroll
      }
    }, 150);
...
const triggerFeedback = (type, value) => {
  let messages = [];
  if (type === 'box') {
     messages = ['Starke Wahl.', 'Mehr Spielraum gewählt.', 'Einkaufsvorteil gesichert.', 'Sehr gute Entscheidung.'];
  } else if (type === 'type' && value === 'single') {
     messages = ['Sauberer Einstieg.', 'Direkt sichern.', 'Deine Box ist bereit.'];
  } else if (type === 'type' && value === 'abo') {
     messages = ['Upgrade aktiviert.', 'Deal-Vorsprung gesichert.', 'Legacy-Vorteile freigeschaltet.', `Punkte-Faktor x${activeLevel.value} aktiv.`];
  }
...
const selectBox = (id) => {
  if (activeLevel.value !== id) {
     ...
     triggerFeedback('box', id);
  }
};

const selectType = (type) => {
  if (purchaseType.value !== type) {
...
const isAboMode = computed(() => purchaseType.value === 'abo');

const levels = [
  { id: 1, price: '111', value: '220–330' },
  { id: 2, price: '222', value: '440–660' },
  { id: 3, price: '333', value: '660–990' },
  { id: 4, price: '444', value: '880–1.320' },
  { id: 5, price: '555', value: '1.100–1.650' }
];

const activeLevelData = computed(() => levels.find(l => l.id === activeLevel.value));
...
const toggleBodyLegacyClass = (isAbo) => {
  if (isAbo) {
...
watch(isAboMode, (newVal) => {
  toggleBodyLegacyClass(newVal);
});

onMounted(() => {
  const p = new URLSearchParams(window.location.search);
  const lv = parseInt(p.get('level'));
  const tp = p.get('type');
  if (lv >= 1 && lv <= 5) activeLevel.value = lv;
  if (tp === 'single' || tp === 'abo') purchaseType.value = tp;
  
  toggleBodyLegacyClass(isAboMode.value);
});
...
const checkout = () => {
  router.post(route('stripe.checkout'), { level: activeLevel.value, type: purchaseType.value });
};
